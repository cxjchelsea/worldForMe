...
6. 来自哪些 QC1.1 传统／文本，各自 `source_status` 是什么？
7. 何时开始超出单一来源身份？
8. 被后世怎样反复调用、改写或反转？
...
正式关系使用 `qc2_component_relation`，但只有 target 已通过自身 QC2 准入后才能创建；candidate 不能因本专题发现而自动升级。
...
该结论只验证 archetype 模板；整个 QC2 Template V0 仍需等待 plot_pattern 与 symbol Pilot 后才能进入 V1 Freeze Review。