# QC2｜世界文化母题、原型与叙事结构模板总则 V0

> 状态：`TEMPLATE_V0_REVISED_AFTER_ARCHETYPE_PILOT_B_PASS_A`
>
> 适用范围：QC2 横向抽象层中的 motif / archetype / plot_pattern / symbol 专题。
>
> 设计依据：QC1.1.1 希伯来—圣经叙事传统与 QC1.1.2 希腊—罗马神话传统两个已验收来源专题；由“洪水与灾后重建” motif Pilot A、Shared Data Layer Pass，以及“受苦义人” archetype Pilot B Content Pass A 持续反向修订。

---

# 一、QC2 的职责

QC2 不再研究“某一文明内部有什么故事”，而研究：
...
每个 QC2 对象必须尽可能回指 QC1.1，并显式保存 `source_status`。
...
这允许 QC2 先发现跨文化组件，又不会反向冒充来源层已经建成。
...
Shared Data Layer Pass 后，QC2 正式固定三种最小数据实体：
...
完整 schema：[[QC2｜共享数据层规范 V0]]

共享聚合入口：[[QC2｜共享数据.base]]

## 7.1 qc2_source_reference

回答：

> 这个组件从哪个 QC1.1 来源传统／故事／文本中抽取？
...
> 两个正式 QC2 component 之间存在什么语义关系？

只有 target 已经通过对应类型准入、成为正式 QC2 component 后，才创建正式 relation entity。候选 archetype / plot_pattern / symbol 不因另一个 Pilot 的发现而提前升级。
...
`QC2_TEMPLATE_STATUS = V0_DRAFT / NOT_FROZEN`
