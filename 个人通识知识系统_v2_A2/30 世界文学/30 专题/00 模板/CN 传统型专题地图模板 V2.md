---
id: WL-TEMPLATE-CN-TRADITION-V2
type: literature_topic_template
name: CN 传统型专题地图模板 V2
status: FROZEN_V2_1
scope: CN narrative/cultural tradition topics
validated_by:
  - CN1.1.1 希伯来—圣经叙事传统
  - CN1.1.2 希腊—罗马神话传统
---
...
> 适用于 CN 中以“某套文化叙事传统如何形成、保存、定型、传播与被重写”为核心问题的专题。V2.1 统一 R/T/M 的产品接口，同时保留 CN 自己的内部知识结构和研究数据层。
...
专题归属由 scoped metadata 控制，例如 CN1.1.1：
...
同一作品可以同时拥有 T/R/M/G/Q/CN 等多个专题的 scoped metadata，互不覆盖。
...
负责与 CN 的 motif / archetype / plot_pattern / symbol 等对象连接，并严格区分：结构相似、可能传播、历史传播、明确引用、人物／名称借用、直接改编。
...
一个 CN 传统型专题只有同时满足以下条件才可标记 Product PASS：
...
11. 历史研究数据可以保留 legacy provenance，但产品层使用当前 CN 编号；
...
REFERENCE_TOPIC_1 = CN1.1.1
REFERENCE_TOPIC_2 = CN1.1.2
REFERENCE_TOPIC_WORK_POOL = 20 + 20（当前参考实现，不是全局硬性数量标准）
BULK_REUSE_FOR_CN1.1.3_TO_CN1.1.11 = AUTHORIZED_AFTER_PER_TOPIC_CONTENT_REVIEW
TY8.2_TY8.3_MIGRATION = OUT_OF_SCOPE
```
