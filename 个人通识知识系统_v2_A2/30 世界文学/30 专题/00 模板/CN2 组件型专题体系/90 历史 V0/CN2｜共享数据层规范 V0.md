...
>
> 适用范围：CN2 的 motif / archetype / plot_pattern / symbol 四类组件。
>
...
只有 target 已经成为正式 CN2 component 时才创建正式 `qc2_component_relation`。
...
```text
CN2_SHARED_DATA_LAYER = ESTABLISHED_V0 / CROSS_TYPE_REVIEWED
CN2_SOURCE_REFERENCE_SCHEMA = ACTIVE / CROSS_TYPE_PASS
CN2_COMPONENT_RELATION_SCHEMA = ACTIVE_WITH_PROMOTION_GATE / STRUCTURALLY_ACCEPTED
CN2_REAL_CROSS_TYPE_COMPONENT_RELATION_EVIDENCE = DEFERRED_BY_MEANINGFUL_TARGET_GATE
CN2_WORK_REFERENCE_SCHEMA = ACTIVE / CROSS_TYPE_PASS
CN2_FIGURE_REWRITING_RELATION = ACTIVE
CN2_STRUCTURAL_SIMILARITY_WORK_RELATION = ACTIVE
CN2_PLOT_PATTERN_SLOT_FIELDS = ACTIVE_OPTIONAL_EXTENSION
CN2_SYMBOL_WORK_MEANING_FIELDS = ACTIVE_OPTIONAL_EXTENSION
CN2_ICONOGRAPHIC_INHERITANCE_RELATION = NOT_PROMOTED / REQUIRES_DOCUMENTED_VISUAL_CHAIN
CN2_RELATION_ATOMICITY = REQUIRED / PASS
CN2_SHARED_BASE = REQUIRED / COMPATIBILITY_PASS

CN2_TEMPLATE_V1_FREEZE_REVIEW = AUTHORIZED_TO_START
CN2_TEMPLATE_V1_FREEZE = NOT_YET_AUTHORIZED
```

详细总复核：[[CN2｜Four-Type Cross-Template Conflict and Shared Data Review]]
