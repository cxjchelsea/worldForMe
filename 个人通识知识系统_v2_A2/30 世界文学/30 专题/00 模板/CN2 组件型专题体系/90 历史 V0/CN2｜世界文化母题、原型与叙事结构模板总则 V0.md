# CN2｜世界文化母题、原型与叙事结构模板总则 V0

> 状态：`TEMPLATE_V0_REVISED_AFTER_ARCHETYPE_PILOT_B_PASS_A`
>
> 适用范围：CN2 横向抽象层中的 motif / archetype / plot_pattern / symbol 专题。
>
> 设计依据：CN1.1.1 希伯来—圣经叙事传统与 CN1.1.2 希腊—罗马神话传统两个已验收来源专题；由“洪水与灾后重建” motif Pilot A、Shared Data Layer Pass，以及“受苦义人” archetype Pilot B Content Pass A 持续反向修订。

---

# 一、CN2 的职责

CN2 不再研究“某一文明内部有什么故事”，而研究：
...
每个 CN2 对象必须尽可能回指 CN1.1，并显式保存 `source_status`。
...
这允许 CN2 先发现跨文化组件，又不会反向冒充来源层已经建成。
...
Pilot B 已证明 `source_status + qc2_source_reference` 可以从 motif 复用到 archetype，而无需重造来源 schema。
...
Shared Data Layer Pass 后，CN2 正式固定三种最小数据实体：
...
完整 schema：[[CN2｜共享数据层规范 V0]]

共享聚合入口：[[CN2｜共享数据.base]]

## 7.1 qc2_source_reference

回答：

> 这个组件从哪个 CN1.1 来源传统／故事／文本中抽取？
...
## 7.2 qc2_component_relation

回答：

> 两个正式 CN2 component 之间存在什么语义关系？

只有 target 已经通过对应类型准入、成为正式 CN2 component 后，才创建正式 relation entity。候选 archetype / plot_pattern / symbol 不因另一个 Pilot 的发现而提前升级。

当前 schema 已建立，真实跨类型 relation 仍等待后续正式 target。

## 7.3 qc2_work_reference
...
Pilot B 当前不直接进入 Acceptance：仍需后世 `qc2_work_reference` 验证，并在适当时机验证正式跨类型 `qc2_component_relation`。
...
- [x] `qc2_source_reference` 已跨 motif / archetype 两类复用
- [ ] `qc2_component_relation` 完成真实跨类型复用
- [ ] `qc2_work_reference` 跨 motif / archetype 两类复用
- [ ] 文本谱系不会复制作品主节点
...
`CN2_TEMPLATE_STATUS = V0_DRAFT / NOT_FROZEN`
