# CN2｜Pilot D 巴别塔 Acceptance Review

> Review object：`WL-TOPIC-CN23-BABEL`
>
> Review type：`symbol`
>
...
```text
CN2_PILOT_D_CONTENT_ACCEPTANCE = PASS
CN2_PILOT_D_SYMBOL_BOUNDARY = PASS
CN2_PILOT_D_ADMISSION_MODEL = PASS
CN2_PILOT_D_STABLE_MEANING_MODEL = PASS
CN2_PILOT_D_SEMANTIC_DRIFT_MODEL = PASS
CN2_PILOT_D_CROSS_MEDIA_PRESSURE = PASS
CN2_PILOT_D_SOURCE_GOVERNANCE = PASS
CN2_PILOT_D_RELATION_GOVERNANCE = PASS
CN2_PILOT_D_WORK_REFERENCE_ACCEPTANCE = PASS
CN2_PILOT_D_COMPONENT_RELATION_GATE = PASS_BY_DEFERRED_GATE
CN2_PILOT_D_ICONOGRAPHIC_INHERITANCE_GATE = PASS_BY_NON_PROMOTION

CN2_PILOT_D_REFERENCE_STATUS
...
巴别塔作为 CN2 symbol 的最低准入，不是“《创世记》里出现了一座塔”，而是：
...
```text
CN2_PILOT_D_SOURCE_OBJECT_VS_SYMBOL_BOUNDARY = PASS
CN2_PILOT_D_ORDINARY_SIMILARITY_GATE = PASS
```
...
```text
CN2_PILOT_D_ADMISSION_EVIDENCE_MODEL = PASS
CN2_PILOT_D_STABLE_MEANING_MODEL = PASS
```
...
CN2_PILOT_D_SEMANTIC_DRIFT_MODEL = PASS
CN2_PILOT_D_CROSS_MEDIA_PRESSURE = PASS
CN2_PILOT_D_INVERTED_CONTESTED_MEANING_GATE = PASS_BY_NO_FORCED_FILL
```
...
```text
CN2_PILOT_D_SOURCE_STATUS_GOVERNANCE = PASS
CN2_PILOT_D_NO_BACK_PROJECTION = PASS
```
...
```text
1 × qc2_source_reference
3 × qc2_work_reference
...
```text
CN2_PILOT_D_SYMBOL_WORK_FIELDS = PASS
CN2_SHARED_DATA_LAYER_BREAKING_CHANGE_REQUIRED = NO
```
...
```text
CN2_PILOT_D_RELATION_GOVERNANCE = PASS
CN2_PILOT_D_ICONOGRAPHIC_INHERITANCE_GATE
...
当前没有为了测试 schema 而强制建立 `qc2_component_relation`。
...
```text
CN2_PILOT_D_COMPONENT_RELATION_GATE
...
```text
CN2_PILOT_D = CLOSED_ACCEPTED
CN2_SYMBOL_TEMPLATE_V0 = VALIDATED_BY_ONE_REFERENCE_SYMBOL
CN2_ALL_FOUR_COMPONENT_TYPES_HAVE_ACCEPTED_REFERENCE_PILOT = YES
