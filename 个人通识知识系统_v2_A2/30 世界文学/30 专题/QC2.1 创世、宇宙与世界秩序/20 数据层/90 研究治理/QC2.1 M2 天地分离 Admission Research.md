# QC2.1｜M2 天地分离｜Admission Research
...
>
> baseline：`QC2 Motif Template V1 FROZEN`
>
...
QC1.1.2 希腊—罗马神话传统已存在。Hesiod《神谱》中 Gaia 生出 Ouranos 覆盖她，Ouranos 将子代压回 Gaia 内部；Cronos 对 Ouranos 的暴力行动结束这一压迫结构。古典注释传统长期把该段作为 Heaven / Earth separation 解释。
...
→ future QC1.1.7 backfill candidate

Māori Rangi-Papa
→ external_source_verified_text_only
→ current QC1.1 taxonomy has no dedicated Oceania topic
```

治理结论：

```text
QC2 source attestation
≠ QC1.1 source topic completion
```

所以不需要等待 QC1.1.7 或未来 Oceania 来源专题建设完毕，才允许 M2 准入；但正式 source reference 必须保留真实 `source_status`，不得冒充 reference_topic。
...
```text
QC2.1_M2_ADMISSION_RESEARCH = PASS
QC2.1_M2_COMPONENT_TYPE = motif
QC2.1_M2_WORKING_NAME = 天地分离
QC2.1_M2_REQUIRED_INVARIANTS = STABLE
QC2.1_M2_SOURCE_PRESSURE_TEST = PASS
QC2.1_M2_BOUNDARY_TEST = PASS
QC2.1_M2_PROMOTE_TO_TOPIC_BUILD_QUEUE = YES
```
...
```text
WL-TOPIC-QC21-SKY-EARTH-SEPARATION
```
