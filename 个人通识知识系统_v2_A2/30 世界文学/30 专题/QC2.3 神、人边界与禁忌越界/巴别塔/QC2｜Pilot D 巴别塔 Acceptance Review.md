...
> Review object：`WL-TOPIC-QC23-BABEL`
>
> Review type：`symbol`
>
...
巴别塔作为 QC2 symbol 的最低准入，不是“《创世记》里出现了一座塔”，而是：
...
```text
QC2_PILOT_D_SOURCE_OBJECT_VS_SYMBOL_BOUNDARY = PASS
QC2_PILOT_D_ORDINARY_SIMILARITY_GATE = PASS
```
...
```text
QC2_PILOT_D_ADMISSION_EVIDENCE_MODEL = PASS
QC2_PILOT_D_STABLE_MEANING_MODEL = PASS
```
...
QC2_PILOT_D_SEMANTIC_DRIFT_MODEL = PASS
QC2_PILOT_D_CROSS_MEDIA_PRESSURE = PASS
QC2_PILOT_D_INVERTED_CONTESTED_MEANING_GATE = PASS_BY_NO_FORCED_FILL
```
...
```text
QC2_PILOT_D_SOURCE_STATUS_GOVERNANCE = PASS
QC2_PILOT_D_NO_BACK_PROJECTION = PASS
```
...
```text
QC2_PILOT_D_SYMBOL_WORK_FIELDS = PASS
...
```text
QC2_PILOT_D_RELATION_GOVERNANCE = PASS
QC2_PILOT_D_ICONOGRAPHIC_INHERITANCE_GATE
...
```text
QC2_PILOT_D_COMPONENT_RELATION_GATE
...
```text
QC2_PILOT_D = CLOSED_ACCEPTED
QC2_SYMBOL_TEMPLATE_V0 = VALIDATED_BY_ONE_REFERENCE_SYMBOL
QC2_ALL_FOUR_COMPONENT_TYPES_HAVE_ACCEPTED_REFERENCE_PILOT = YES
