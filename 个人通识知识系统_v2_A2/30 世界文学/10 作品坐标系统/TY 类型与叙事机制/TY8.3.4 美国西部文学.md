---
id: WL-TY8.3.4
legacy_id: WL-TY8.3.4
type: literature_node
name: "美国西部文学"
code: TY8.3.4
legacy_code: QT8.3.4
axis: TY
system_role: work_coordinate
parent: WL-TY8.3
level: 4
coverage_priority: Interest
node_kind: legacy_migration_gateway
anchorable: false
status: MIGRATED_LEGACY
migrated_to: WL-CN31-CAND-WESTERN-OUTLAW
qt_role: "Western／美国西部文学类型史继续留在 TY"
source_version: "2.12-qt8-restructure-migrated-qc31"
---

# TY8.3.4 美国西部文学｜已迁移

文化模型主入口已迁移到 [[美国西部cowboy与outlaw英雄模型|美国西部 cowboy / outlaw 英雄模型]]。

```text
CN3 → cowboy / outlaw / frontier hero 文化模型
TY  → Western／西部文学类型史
历史 → 边疆扩张、牧牛业、土地与暴力秩序
```

后续不在本旧节点继续新增文化模型内容。
