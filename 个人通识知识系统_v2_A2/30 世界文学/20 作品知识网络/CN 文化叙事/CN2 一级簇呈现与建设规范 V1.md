...
CN2.1–CN2.20 是**一级问题域／母题簇导航容器**，不是单一母题，也不是直接承载全部研究细节的重型专题。

本规范解决三个问题：

1. **人打开 CN2.x 时要能迅速看懂**：这个问题域研究什么、里面有什么组件、来自哪些传统、怎么读；
2. **系统仍保留严谨治理**：component acceptance、source evidence、coverage review、stage freeze、reopen gate 等继续存在，但不占据认知主页主体；
3. **作品层必须可见**：CN2.x 必须提供中央 `40 作品` 的投影视图，使“来源传统—叙事传统—组件—作品”在产品层闭合。
...
4. **它们来自哪些 CN1.1 来源传统？**
5. **它们在哪些 CN1.2 具体叙事传统中被组织、定型或强化？**
...
```text
CN1.1 source_tradition
→ source material / source witness
→ CN2 component
```

CN1.1 可以直接向 CN2 提供 component 候选，不要求先经过 CN1.2。

## 4.2 CN1.2 的职责：具体传统生命史
...
每个 CN2.x 必须存在 `03 作品.base`，只投影中央 `40 作品` 的合法 work 实体；它可以因尚未完成 work backfill 而暂时为空，但**不可以因为当前为空而省略产品入口**。

严格区分：

```text
source witness ≠ work
CN1.1 tradition ≠ work
CN1.2 tradition ≠ work
CN2 component ≠ work
```
...
```text
00 CN2.x 一级簇主页.md
01 CN2.x 组件关系.canvas
02 CN2.x 组件.base
03 CN2.x 作品.base
90 治理/00 CN2.x 治理索引.md
...
不得按 CN2.1 → CN2.2 → … → CN2.20 的顺序机械填满。

---

# 11. CN2.x Stage Freeze 的含义
...
CN2.1 已有六个经过验证的 active components，适合用于冻结一级簇的前台呈现职责；其历史治理材料继续保留为后台证据。