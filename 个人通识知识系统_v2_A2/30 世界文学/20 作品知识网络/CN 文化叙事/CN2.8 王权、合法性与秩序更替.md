---
id: WL-CN2.8
legacy_id: WL-CN2.8
type: literature_node
name: "王权、合法性与秩序更替"
code: CN2.8
legacy_codes:
- CN2.8
- TY8.2.8
namespace: CN
legacy_axis: "Q"
system_role: work_knowledge_network
parent: WL-CN2
level: 4
coverage_priority: Interest
node_kind: taxonomy_cluster
anchorable: true
topic_map: null
source_version: "2.21-qt8.2-solomon-named-archetype-accepted"
status: CONTENT_OPEN
---
 
# CN2.8 王权、合法性与秩序更替

> 路径：作品知识网络 → TY8 → CN2 世界文化母题、原型与叙事结构 → **CN2.8 王权、合法性与秩序更替**

本节点继续作为一级母题簇／问题域容器，不直接承担所有重型研究。其下可以逐步建立 motif / archetype / plot_pattern / symbol 等具体 CN2 component；对象可同时关联其他母题簇，不采用唯一归属树。

## 已建立组件

### 所罗门王｜named archetype Reference Topic

→ [[../../30 专题/CN2.8 王权、合法性与秩序更替/所罗门王/00 所罗门王|所罗门王]]

当前状态：`ACCEPTED_REFERENCE_NAMED_ARCHETYPE_V0`

对象类型：

```text
archetype
→ named_archetype
```

已验证：

- `source_figure` 与 `named_archetype` 分层；
- `required_identity_anchors / supporting_identity_anchors`；
- `core_functions / variable_features` 在命名型对象上的复用；
- 来源层高重要特征不自动等于跨接受史 core function；
- `builder_and_centralizer` 降级为 `temple_builder_and_centralizer / variable_feature`；
- core functions 收束为智慧／裁判、神授王权、卓越知识权威；
- `figure_rewriting` 正式进入共享 work relation vocabulary；
- `character_or_name_borrowing / figure_rewriting / direct_adaptation` 三者边界可执行；
- Jewish / Christian / Islamic / esoteric reception 可在不倒灌来源层的情况下统一建模。

Acceptance Review：

→ [[../../30 专题/CN2.8 王权、合法性与秩序更替/所罗门王/CN2｜Pilot B.1 所罗门王 Acceptance Review|Pilot B.1 所罗门王 Acceptance Review]]

## 后续建设原则

本节点不因为“王者相关”就批量生成文化原型。只有具体对象通过对应模板准入并具有来源／接受证据后，才建立正式 component。

所罗门王的准入验证已经结束；后续扩展属于增量研究，不再承担模板准入压力测试。