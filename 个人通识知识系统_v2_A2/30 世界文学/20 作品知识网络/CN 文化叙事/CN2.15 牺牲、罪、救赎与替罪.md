---
id: WL-CN2.15
legacy_id: WL-CN2.15
type: literature_node
name: "牺牲、罪、救赎与替罪"
code: CN2.15
legacy_codes:
- CN2.15
- TY8.2.15
namespace: CN
legacy_axis: "Q"
system_role: work_knowledge_network
parent: WL-CN2
level: 4
coverage_priority: Interest
node_kind: taxonomy_cluster
anchorable: true
topic_map: null
source_version: "2.19-qt8.2-archetype-pilot-b-accepted"
status: CONTENT_OPEN
---
 
# CN2.15 牺牲、罪、救赎与替罪

> 路径：作品知识网络 → TY8 → CN2 世界文化母题、原型与叙事结构 → **CN2.15 牺牲、罪、救赎与替罪**

本节点继续作为一级母题簇／问题域容器，不直接承担所有重型研究。其下可以逐步建立 motif / archetype / plot_pattern / symbol 等具体 CN2 component；对象可同时关联其他母题簇，不采用唯一归属树。

## 已建立组件

### 受苦义人｜archetype Pilot B

→ [[../../30 专题/CN2.15 牺牲、罪、救赎与替罪/受苦义人/00 受苦义人|受苦义人]]

当前状态：`ACCEPTED_REFERENCE_ARCHETYPE_V0`

对象类型：

```text
archetype
→ abstract_archetype
```

本 Pilot 已通过：

- archetype 与 theme / trait / plot_pattern 的边界验证；
- `core_functions / variable_features` 验证；
- source figure 与 archetype 分层；
- 《约伯记》与 `Ludlul bēl nēmeqi` 的跨来源 functional comparison；
- `source_status` 在 archetype 上的跨类型复用；
- functional similarity 与 historical transmission 分离；
- Shared Data Layer V0 的 `qc2_source_reference` 复用；
- Shared Data Layer V0 的 `qc2_work_reference` 复用；
- Joseph Roth《Job》、MacLeish《J.B.》、Spark《The Only Problem》三种后世调用方式核证；
- component relation target gate。

正式 `qc2_component_relation` 继续等待 target component 自身完成准入。

## 下一阶段

按照 CN2 Pilot Plan，archetype 类型下一步进入：

```text
所罗门王
→ named_archetype pressure test
```

用于验证命名型文化原型与 CN1.1 来源人物能否稳定分层。

## 后续建设原则

本母题簇不因为概念上相关就批量生成组件。只有当具体对象能够通过对应模板准入并具有来源／文本证据时，才建立正式 component。
