...
QT8.2 研究不同来源传统中反复出现、能够跨文本与跨文化比较的叙事组件，回答它们如何被定义、形成、传播、重写、组合并持续获得新意义。
...
QT8.2 是横向抽象层，不负责重建单一文明的完整来源史，不把 theme 纳入核心对象，不把结构相似自动解释为历史传播。
...
Pilot A / B / B.1 / C / D 仅表示模板验证顺序，不等于 QT8.2.x 的建设顺序。
...
每个 component 应尽可能回指 QT8.1，并保存 `source_status`：
...
解释层与数据层保持分离；共享 Base：[[QT8.2｜共享数据.base]]。
...
正式 `qt82_component_relation` 只有在：
...
V1 冻结时真实记录仍为 `0 × qt82_component_relation`。该缺口被接受为：
