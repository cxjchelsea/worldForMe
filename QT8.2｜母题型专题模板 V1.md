...
> 对象类型：`motif`
>
> 状态：`QT8.2_MOTIF_TEMPLATE_V1_FROZEN`
>
...
按 QT8.1 来源传统组织，所有来源使用共享 `qt82_source_reference` 并记录 `source_status`。
...
只有 target 已正式准入且关系具有解释价值时，才创建 `qt82_component_relation`。
...
统一使用 `qt82_work_reference`。重点记录保留了哪些 invariants、修改了哪些 slots，以及 relation_type / evidence_level。
